...
def acomodar(s: list[str]) -> list[str]:
  lista_acomodada: list[str] = list()
  up: int = contar(s,'UP')
  lla: int = contar(s,'LLA')
  for i in range(0,up):
    lista_acomodada.append('UP')
  for i in range(0,lla):
    lista_acomodada.append('LLA')
  return lista_acomodada

def contar(s: list[str], partido: str) -> int:
  contador = 0
  for boleta in s:
    if boleta == partido:
      contador += 1 
